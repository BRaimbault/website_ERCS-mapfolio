...
function callModal (thumbnail) {
    ...
    var previewImgSrc = "https://dl.dropboxusercontent.com/u/59360120/Ethiopia_Mapfolio-Website/thumb/" + $(thumbnail).attr("data-filename").slice(0,-4)+'_thumb.jpg'
...
function getCentroids() {
  d3.csv("https://dl.dropboxusercontent.com/u/59360120/Ethiopia_Mapfolio-Website/ethiopia-maps.csv", function(data){
...
function generatepreviewhtml(data){
    ...
    $.each(data, function(index, item){
        var pdfSrc = item.pdflink;
        var link = '';
        if (item.link){
            link = '<p style="font-size:small; margin:0 0 0 10px;"><b>Source:</b> <a href="'+item.link+'" target="_blank">'+item.link+'</a></p>';
        };
        var itemhtml = '<div id="'+item.thumbnail_id+'" style="display:none," class="thumbnailWrap ALL-EXTENT ALL-SECTOR mapped '+
          item.extent+' '+item.sector+'" data-filename="'+ item.filename +'">' +
